...
STDOUT="docs/evidence/phase6_compose_load_smoke_${RUN_ID}.txt"
RUNTIME="docs/evidence/phase6_compose_load_smoke_runtime_${RUN_ID}.txt"

"$SCRIPT_DIR/sync_to_vm.sh"

set +e
"$SCRIPT_DIR/run.sh" "mkdir -p docs/evidence reports/generated; start=\$(date +%s); set +e; {
  set -euo pipefail
  ...
  if [ ! -f .env ]; then
    printf '# VM-only default env for Codex Compose load smoke. No secrets.\\n' > .env
  fi

  export COMPOSE_PROJECT_NAME=credit_mlops_codex_load
  cleanup() {
  ...
  API_BASE_URL=http://127.0.0.1:8000 \
    BENCHMARK_SCOPE=docker_compose_core_stack \
    BENCHMARK_FAIL_ON_ERROR=1 \
    BENCHMARK_OUTPUT_NAME=property_api_docker_compose_benchmark_${RUN_ID}.json \
    uv run python scripts/property_api_external_benchmark.py
} 2>&1 | tee $STDOUT; status=\${PIPESTATUS[0]}; set -e; end=\$(date +%s); printf 'compose_load_smoke_exit=%s\\nruntime_seconds=%s\\n' \"\$status\" \"\$((end-start))\" | tee $RUNTIME; exit \"\$status\""
status=$?
set -e
