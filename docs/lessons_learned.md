...
- A passing read-only cloud smoke still does not prove apply readiness. Keep placeholder image tags, secret population, apply-time IAM, cost approval, and post-deploy URL/rollback evidence as separate gates.
- A cloud smoke that passes read-only checks still does not prove Terraform apply readiness. Keep placeholder image tags, secret population, apply-time IAM, cost approval, and post-deploy URL/rollback evidence as separate gates.
- Docker health is not enough for domain readiness. The Compose API was healthy while AVM returned 503 until `PROPERTY_GOLD_PATH` pointed at a mounted VM-only gold parquet.
