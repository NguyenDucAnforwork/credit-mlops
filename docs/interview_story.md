...
- Resolved the registry gate with a targeted Terraform apply that added exactly one repository, pushed both existing images, replaced placeholder tags with SHA256 digest references, and stopped after a final 29-resource plan without deploying Cloud Run.
